...
def mine_on_message(ws, message):
    ...
    if is_candle_finished:
        ...
        if len(closes) >= RSI_CYCLE:                                            # Calculate after RSI cycle completed
            ...
            print(rsi)                                                          # Will preing the rsi values
            ...
            if rsi[-1] < RSI_OVERSOLD:
                if in_position:
                    ...
                    order_result = execute_order(SIDE_BUY, HOW_MUCH_TO_BUY, TRADE_SYMBOL, ORDER_TYPE_MARKET)
            ...
            if rsi[-1] > RSI_OVERBOUGHT:
                if in_position:
                    ...
                    # Execute order in binance API
                    order_result = execute_order(SIDE_SELL, HOW_MUCH_TO_BUY, TRADE_SYMBOL, ORDER_TYPE_MARKET)
...
"""
Function role is to execute the buy/sell order through Binance API.
* Side = buy/sell order
* Symbol = the symbol of the coin that we want to buy
* Quantity = the quantity of the coin that we ant to buy
"""
def execute_order(side, quantity, symbol, order_type):
    try:
